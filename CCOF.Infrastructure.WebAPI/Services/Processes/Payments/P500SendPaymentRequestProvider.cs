...
public class P500SendPaymentRequestProvider(IPaymentValidator paymentvalidator,IOptionsSnapshot<ExternalServices> bccasApiSettings, ID365AppUserService appUserService, ID365WebApiService d365WebApiService, ID365DataService dataService, ILoggerFactory loggerFactory, TimeProvider timeProvider) : ID365ProcessProvider
{
    ...
    public string RequestPaymentLineUri
    {
        get
        {
            ...
            // """;
            var requestUri = $"""
                         ofm_payments?$select=ofm_paymentid,_ofm_organization_value,ofm_name,createdon,ofm_amount,ofm_description,ofm_effective_date,_ofm_fiscal_year_value,ofm_revised_invoice_received_date,ofm_revised_invoice_date,ofm_revised_effective_date,_ofm_funding_value,ofm_invoice_line_number,_owningbusinessunit_value,ofm_payment_type,ofm_remittance_message,statuscode,ofm_invoice_number,_ofm_application_value,ofm_siteid,ofm_payment_method,ofm_supplierid,ofm_invoice_received_date,ofm_invoice_date&$expand=ccof_programyear($select=ccof_programyearnumber,ccof_name),ofm_application($select=ofm_application),ofm_facility($select=accountnumber,name)&$filter=(contains(owningbusinessunitname, 'CCOF') and statuscode eq {(int)OfM_Payment_StatusCode.ApprovedforPayment} and ofm_supplierid ne null and ofm_siteid ne null and ofm_payment_method ne null and ofm_amount ne null and (ofm_invoice_date eq '{localDateOnlyPST}' or ofm_revised_invoice_date eq '{localDateOnlyPST}'))&$orderby=ofm_name asc
                         """;
    ...
    public async Task<JsonObject> RunProcessAsync(ID365AppUserService appUserService, ID365WebApiService d365WebApiService, ProcessParameter processParams)
    {
        ...
        try
        {
            ...
            List<Ccof_Programyear> fiscalYears = [.. JsonSerializer.Deserialize<List<Ccof_Programyear>>(fiscalYearsData.Data)];
            ...
            ////var serializedPFXData = JsonSerializer.Deserialize<List<>>(latestPaymentFileExchangeData.Data.ToString());
